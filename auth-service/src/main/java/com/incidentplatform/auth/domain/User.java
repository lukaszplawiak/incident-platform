...
public class User {
     ...
     * Set to true only after {@link #enableMfa(String)} is called with
     * a verified TOTP code.
     */
     ...
     * NULL until MFA is enabled. Decrypted by {@code AesEncryptionService}
     * at verify time — never stored or logged as plain text.
     */
    @Column(name = "mfa_secret")
    private String mfaSecret;

    /**
     * Temporary encrypted secret during MFA setup.
     * Generated by POST /auth/mfa/setup, moved to {@link #mfaSecret}
     * on successful POST /auth/mfa/enable, then cleared.
     */
    @Column(name = "mfa_pending_secret")
    ...
    /**
     * Stores the pending secret during MFA setup (step 1).
     * The secret is AES-256-GCM encrypted before being passed here.
     */
    public void storePendingMfaSecret(String encryptedSecret) {
        this.mfaPendingSecret = encryptedSecret;
        this.updatedAt = Instant.now();
    }
    ...
    public void enableMfa() {
        if (this.mfaPendingSecret == null) {
            throw new IllegalStateException(
                    "Cannot enable MFA — no pending secret. Call setup first.");
        }
        this.mfaSecret = this.mfaPendingSecret;
    ...
    /**
     * Disables MFA — clears secret and resets state.
     * Called by POST /auth/mfa/disable after verifying password + TOTP.
     */
